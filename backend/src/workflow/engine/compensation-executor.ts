...
export type CompensationAction =
  ...
  | { type: 'http'; method: 'POST' | 'PUT' | 'DELETE'; url: string; body?: any; headers?: Record<string, string> }
...
export class CompensationExecutor {
  ...
  async execute(action: CompensationAction, context: { variables: Record<string, any> }, logs: string[]) {
    switch (action.type) {
      case 'deleteVar':
  ...
  private async executeHttp(
    action: Extract<CompensationAction, { type: 'http' }>,
    logs: string[]
  ) {
