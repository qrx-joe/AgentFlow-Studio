...
export const useWorkflowEdgeHandlers = (workflowStore: any, addEdges: (edges: any[]) => void) => {
  ...
  const handleConnect = (params: any) => {
    ...
    if (sourceNode && sourceNode.type !== 'condition') {
      const existingOutgoing = workflowStore.edges.filter((edge: any) => edge.source === params.source).length
      if (existingOutgoing >= 1) {
        ElMessage.warning('该节点已存在出边，请确认是否需要多分支')
      }
    }
    const outgoingCount = workflowStore.edges.filter((edge: any) => edge.source === params.source).length
  ...
  const handleEdgeClick = (_: any, edge: any) => {
    ...
    const siblings = workflowStore.edges.filter((item: any) => item.source === edge.source && item.id !== edge.id)
    if (nextLabel === 'True') {
      sourceNode.data = {
        ...sourceNode.data,
        trueEdgeId: edge.id,
